...
// Prepare and create Eventbrite event from event card + brief data
app.post('/api/prepare-eventbrite-event', async (req, res) => {
  try {
    if (!EVENTBRITE_API_KEY || !EVENTBRITE_ORG_ID) {
      return res.status(500).json({ error: 'Eventbrite credentials not configured' });
    }
    ...
    const eventId = response.data.id;
    res.json({ success: true, eventId, eventUrl: `https://www.eventbrite.com/e/${eventId}` });
